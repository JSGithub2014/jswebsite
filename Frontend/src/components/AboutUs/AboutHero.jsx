...
function AboutMain() {
  ...
  const isInViewMobile = useInView(refMobile, { once: false });
  const isInViewDesktop = useInView(refDesktop, { once: false });

  return (
    <div className='flex flex-col md:flex-row w-full h-[calc(100vh-60px)] pt-24 lg:pt-0 md:pt-0 md:px-12'>
      {/* Mobile Layout */}
      <section className="flex flex-col justify-center items-center md:hidden w-full h-full p-6 text-center" ref={refMobile}>
        ...
        <motion.div 
          ...
          animate={isInViewMobile ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
          transition={{ duration: 0.5 }}
        >
          <img src={meetOurTeam} alt="Meet Our Team - Talented Individuals Driving Innovation" className='max-w-full object-contain mb-4' />
        </motion.div>
      ...
      <motion.div 
        ...
        animate={isInViewDesktop ? { opacity: 1, x: 0 } : { opacity: 0, x: -100 }}
        transition={{ duration: 0.5 }}
      >
        <img src={meetOurTeam} alt="Meet Our Team - Talented Individuals Driving Innovation" className='max-w-full max-h-full object-contain mt-24 hidden md:block' />
      </motion.div>
